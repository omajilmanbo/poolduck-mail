...
2. **#20**：建立 CI 基础工作流。
3. **#21**：实现数据库迁移基础与初始模型。
4. **#22**：实现租户登录与用户认证 API。
5. **#33**：认证与租户上下文中间件最小实现。
6. **#23**：实现订阅状态检查与扫码发送限制基础。
7. **#24**：实现 location 与人员映射只读 API。
8. **#25**：实现扫码事件创建与固定邮件任务生成 API。
9. **#26**：实现邮件 sandbox provider 与发送触发 API。

### 业务实现依赖关系（按当前执行计划）

- #31 审查结论合并后，方可进入实现阶段，避免基线漂移。
- #20 提供 CI 最小门禁，保障后续实现可持续验证。
...
- 当前已进入实现阶段：按“推荐的 Issue 执行顺序”推进，优先完成 #20/#21 等基础任务。
