...
4. **#20**：建立 CI 基础工作流（已存在 Issue，非业务实现依赖）。
5. **#21**：Sandbox Mail Provider 与邮件任务最小闭环（含初始数据模型落地：Prisma schema/首次结构迁移，不含历史数据迁移）。
6. **#22**：登录 API（`tenant_id` + email + password）。
7. **#23**：订阅检查与门禁（`trial` / `active` / `expired` / `suspended`）。
8. **#24**：租户 + location + scan_code 映射查询 API。
9. **#25**：扫码事件与邮件任务联动 API。
10. **#26**：发送/重试 API 与审计日志补全。
11. **新建 Issue（待创建）**：认证与租户上下文中间件最小实现（遵循 ADR-003）。

### 业务实现依赖关系（#21～#26 + 新建认证 Issue）

- 新建“认证与租户上下文中间件”Issue 为登录态与 tenant scope 前置，#22/#23/#24/#25/#26 均依赖该 Issue。
- #21 为邮件任务执行基础，#25/#26 依赖 #21。
- #22 为登录 API 主流程，#23/#24/#25/#26 依赖 #22 的会话上下文。
- #23 订阅门禁是 #25/#26 的前置门禁。
- #24 提供映射查询能力，是 #25 的直接前置。
- #25 完成扫码→任务创建闭环后，#26 才实现发送与重试。

> 说明：Issue 优先级不以编号排序；#20 为 CI 任务。MVP 阶段“不需要数据迁移”指不处理历史存量数据迁移；仍需要落地初始结构模型（建表/索引/约束）以支撑 #21～#26 开发。
