...
MVP 核心业务链路已完成本地和 Staging 冒烟验证；当前实现还包含自动邮件任务处理、地点/人员停用与
14 天可恢复删除、operator 地点权限、租户内审计与历史导出，以及 HttpOnly Cookie 会话。当前仍使用
mock/sandbox 邮件 provider 和合成测试数据；真实邮件投递、平台级管理、生产级备份监控及正式对外发布仍需后续迭代。
