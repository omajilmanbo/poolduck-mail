...
| Security（OCI NSG） | `poolduck-mail-stg-web-nsg` / `poolduck-mail-stg-db-nsg` | Staging 入站/出站访问控制 | Local firewall | Terraform 已实施 | TBD | ingress/egress rule summary | `22 admin CIDR` / `80,443 approved web CIDR` / `5432 staging subnet only` | `docs/inventory/` + IaC | 人工指定 | 不得擅自扩大 Web/SSH 来源范围 |
