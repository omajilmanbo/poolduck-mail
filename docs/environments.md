...
- Temporary frontend URL: `http://<staging-public-ip>:3000`.
- Temporary backend URL: `http://<staging-public-ip>:3001`.
...
| `API_BASE_URL` | `http://<staging-public-ip>:3001` before domain; HTTPS API URL after domain | No | Server/API reference. |
| `NEXT_PUBLIC_API_BASE_URL` | Same public API URL used by the frontend | No | Required by the current frontend client. |
| `CORS_ORIGIN` | `http://<staging-public-ip>:3000` before domain; HTTPS frontend URL after domain | No | Do not use wildcard origins. |
