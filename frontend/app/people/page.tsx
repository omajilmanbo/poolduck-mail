...
import { LocationItem, PersonMapping, createApiClient } from '../../src/api/client';
import PersonActionCodesDialog from './PersonActionCodesDialog';

export function canManagePeople(role: string) { return role === 'tenant_manager' || role === 'operator'; }
...
export default function PeoplePage() {
  ...
  useEffect(() => {
    ...
    api.getPeople('cookie', locationId).then(setPeople).catch((cause) => setError(String(cause)));
  }, [api, locationId]);
  ...
  async function save(event: FormEvent) {
    ...
    setError('');
    try {
      if (editingId) await api.updatePerson(locationId, editingId, { person_name: personName, email });
      else await api.createPerson(locationId, { person_name: personName, email });
      reset();
      setPeople(await api.getPeople('cookie', locationId));
  ...
  function reset() { setEditingId(''); setPersonName(''); setEmail(''); }
